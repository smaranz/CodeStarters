...
function AdminScanner() {
            ...
            codeReader.decodeFromVideoDevice(undefined, videoRef.current, async (result, err) => {
                if (result) {
                    ...
                    try {
                        ...
                        const msg = e instanceof Error ? e.message : "Failed to lookup participant";
                        setError(msg);
                    }
                }
    ...
    const handleRedeem = async () => {
        ...
        try {
            ...
            if (pizzaCount > 0) {
                redemptions.push({ participant_id: scannedParticipant.id, kind: "pizza", quantity: pizzaCount, redeemed_by: user?.id || null });
            }
            if (drinkCount > 0) {
                redemptions.push({ participant_id: scannedParticipant.id, kind: "drink", quantity: drinkCount, redeemed_by: user?.id || null });
            }
            if (chipsCount > 0) {
                redemptions.push({ participant_id: scannedParticipant.id, kind: "chips", quantity: chipsCount, redeemed_by: user?.id || null });
            }

            const { error: insertError } = await supabase.from("firehacks_meal_redemptions").insert(redemptions);
            ...
            setError(err instanceof Error ? err.message : "Failed to submit redemption");
        } finally {
            ...
            {scannedParticipant && (
                <div className="bg-white p-8 rounded-[2rem] border border-brand-100 shadow-xl shadow-brand-500/5 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="space-y-6 mb-8">
                        <CounterRow icon={<Pizza className="w-6 h-6" />} color="text-orange-500" label="Pizza Slices" value={pizzaCount} setValue={setPizzaCount} />
                        <CounterRow icon={<CupSoda className="w-6 h-6" />} color="text-blue-500" label="Drinks" value={drinkCount} setValue={setDrinkCount} />
                        <CounterRow icon={<PackageOpen className="w-6 h-6" />} color="text-amber-500" label="Chips" value={chipsCount} setValue={setChipsCount} />
                    </div>
...
function CounterRow({ icon, color, label, value, setValue }: { icon: React.ReactNode; color: string; label: string; value: number; setValue: (n: number) => void }) {
    return (
        <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl">
            <div className="flex items-center gap-4">
                <div className={`w-12 h-12 bg-white rounded-xl flex items-center justify-center shadow-sm ${color}`}>
                    {icon}
                </div>
                <div>
            ...
            <div className="flex items-center gap-4 bg-white p-1 rounded-xl shadow-sm border border-slate-100">
                <button onClick={() => setValue(Math.max(0, value - 1))} className="w-10 h-10 flex items-center justify-center rounded-lg hover:bg-slate-50 text-slate-600 font-bold text-xl">-</button>
                <span className="w-6 text-center font-bold text-slate-900">{value}</span>
                <button onClick={() => setValue(value + 1)} className="w-10 h-10 flex items-center justify-center rounded-lg hover:bg-slate-50 text-slate-600 font-bold text-xl">+</button>
            </div>
        </div>
    );
}
